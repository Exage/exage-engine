...
Tests live alongside their implementations. `src/game/` and scenes have not been implemented yet.
...
Public exports currently include `Engine`, `EngineOptions`, `Time`, `Input`, `Vector2`, `Transform`, `Entity`, `Renderer`, and `TextOptions`.
...
With `Vector2`, `Transform`, and `Entity` implemented, the remaining foundations are `Scene`, `Player`, and `GameScene`. The intended relationship is:
